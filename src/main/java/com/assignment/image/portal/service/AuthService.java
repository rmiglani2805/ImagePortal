...
public class AuthService {
	...
	/**
	 * This makes Imgur OAuth2 authorization code
	 ...
	 *     in the future.
	 * @throws ImageAuthException unable to refresh tokens
	 */
	public String setAuthorizationCode( String authCode ) throws ImageAuthException {
		this.oAuth2 = null;
	 ...
	 * @throws ImageAuthException
	 */
	public boolean isAccessTokenValid()  throws ImageAuthException {
		if( oAuth2 == null ) {
			return false;
		} // if
		Call<Object> call =
	 ...
	 * @throws ImageApiException the account couldn't be loaded
	 */
	public Account getAuthenticatedAccount() throws ImageApiException {
		if( oAuth2 == null ) {
			return null;
		} // if
	...
	private boolean tradeAuthCodeForTokens( String authCode ) throws ImageAuthException {
		...
		try {
			...
			oAuth2 = res.body();
			if( oAuth2 == null ) {
				throw new ImageAuthException( res.message(), res.code() );
			} // if

			return true;
		} catch (IOException e) {
			throw new ImageAuthException( "Error fetching tokens from authorization code: " + e.getMessage() );
		} 

	} // tradeAuthCodeForTokens
	
	private boolean updateAccessToken() throws ImageAuthException {
		if( oAuth2 != null && !oAuth2.isExpiringSoon() ) {
			return true; // nothing to do! all's well as far as we know.
		} // if
		
		if( refreshToken == null ) {
			throw new ImageAuthException( "Cannot update OAuth2 access token; need refreshToken or authorizationCode to be set. See setRefreshToken() or setAuthorizationCode().  User-authenticated calls will not work until then.");
		} // if
		...
		try {
			...
			throw new ImageAuthException( "Error updating access tokens from refresh token: " + e.getMessage() );
		} 
	} // updateAccessToken

	/**
	 * Returns the ImageClient (mostly internal use)
	 * @return the client
	 */
	 ...
	 * @throws ImageAuthException couldn't fetch an access token
	 */
	public void setRefreshToken( String refreshToken ) throws ImageAuthException {
	...
	protected AuthService( ImageClient client, String clientId, String clientSecret ) {
		this.client = client;
